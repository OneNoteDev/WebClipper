...
class PdfPageSelectionRadioButton extends ComponentBase<{}, ClipperStateProp> {
	...
	getAllPagesButton(selectedTabIndex, unselectedTabIndex): any {
		...
		return (
			<div role="radio" id={Constants.Ids.radioAllPagesLabel} className="pdf-control" {...this.enableInvoke({handleMethod: this.onSelectionChange, tabIndex: pdfPreviewInfo.allPages ? selectedTabIndex : unselectedTabIndex, args: true, setNameForArrowKeyNav: Constants.AriaSet.pdfPageSelection})}>
	...
	getPageRangeButton(selectedTabIndex, unselectedTabIndex): any {
		...
		return(
		<div id={Constants.Ids.radioPageRangeLabel} className="pdf-control" {...this.enableInvoke({handleMethod: this.onSelectionChange, tabIndex: !pdfPreviewInfo.allPages ? selectedTabIndex : unselectedTabIndex, args: false, setNameForArrowKeyNav: Constants.AriaSet.pdfPageSelection})}>
				<div className={"pdf-indicator pdf-radio-indicator"}>
				...
				<input
					...
					value={this.props.clipperState.pdfPreviewInfo.selectedPageRange} {...this.enableInvoke({handleMethod: this.onSelectionChange, args: false})}>
				</input>
