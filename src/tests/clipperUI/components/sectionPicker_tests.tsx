...
function initializeClipperStorage(notebooks: string, curSection: string, userInfo?: string) {
	mockStorage = { };
	Clipper.Storage.setValue(Constants.StorageKeys.cachedNotebooks, notebooks);
	Clipper.Storage.setValue(Constants.StorageKeys.currentSelectedSection, curSection);
	Clipper.Storage.setValue(Constants.StorageKeys.userInformation, userInfo);
}
...
test("retrieveAndUpdateNotebookAndSectionSelection should update states correctly when there's notebook, but no curSection information found in storage," +
	...
	Clipper.Storage.getValue(Constants.StorageKeys.currentSelectedSection, (curSection1) => {
		...
		retrievePromise.then((response: SectionPickerState) => {
			Clipper.Storage.getValue(Constants.StorageKeys.cachedNotebooks, (notebooks) => {
				Clipper.Storage.getValue(Constants.StorageKeys.currentSelectedSection, (curSection2) => {
					strictEqual(notebooks, JSON.stringify(freshNotebooks),
						"After fresh notebooks have been retrieved, the storage should be updated with them. In this case, nothing should have changed.");
...
test("retrieveAndUpdateNotebookAndSectionSelection should update states correctly when there's notebook and curSection information found in storage," +
	...
	(error) => {
		Clipper.Storage.getValue(Constants.StorageKeys.cachedNotebooks,
		(notebooks) => {
			Clipper.Storage.getValue(Constants.StorageKeys.currentSelectedSection,
			(curSection) => {
