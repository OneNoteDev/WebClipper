import {Experiments, Feature} from "../scripts/experiments";

import {TestModule} from "./testModule";

export class ExperimentsTests extends TestModule {
	protected module() {
		return "experiments";
	}
	...
	protected tests() {
		...
		test("isFeatureEnabled should return False (OFF) for a valid Feature, but an undefined array of flights", () => {
			let fakeFlights;
			ok(!Experiments.isFeatureEnabled(Feature.DummyExperiment, fakeFlights), "isFeatureEnabled returned True (ON) for Feature that should be OFF because of blank flights");
		});

		test("isFeatureEnabled should return False (OFF) for an empty array of flights", () => {
			let fakeFlights = [];
			ok(!Experiments.isFeatureEnabled(Feature.DummyExperiment, fakeFlights), "isFeatureEnabled incorrectly returned True (ON) for an empty flights array");
		});
		...
		test("isFeatureEnabled should return False (OFF) for a valid Feature whose corresponding numberline doesn't exist in the array of flights", () => {
			...
			ok(!Experiments.isFeatureEnabled(Feature.DummyExperiment, fakeFlights), "isFeatureEnabled returned True (ON) for Feature that should be OFF because it does not appear in the array of flights");
		});
		...
		test("isFeatureEnabled should return True (ON) for a valid Feature and when the corresponding numberline exists in the array of flights", () => {
			...
			ok(Experiments.isFeatureEnabled(Feature.DummyExperiment, fakeFlights), "isFeatureEnabled returned False (OFF) for Feature that should be ON");
		});
		...
		test("isFeatureEnabled should return False (OFF) for a valid Feature in Experiments, and the corresponding numberline is a substring of an individual flight in the flights passed in", () => {
			...
			ok(!Experiments.isFeatureEnabled(Feature.DummyExperiment, fakeFlights), "isFeatureEnabled incorrectly returned true when the numberline itself was not in flights, but it was a substring of another numberline");
		});
	}
}
