...
### Missing Features
6. **Self-contained sign-in (eliminates old sidebar entirely)** — Move sign-in into the renderer popup so the old injected sidebar (clipperInject.ts → clipper.tsx → Mithril) is no longer needed. Worker opens renderer directly on button click (no content script injection). Renderer shows MSA + OrgId sign-in buttons with grayed-out clip controls when not signed in. Sign-in popup launched via `chrome.windows.create` with the existing OAuth URL, result captured on redirect. After sign-in, renderer enables controls and starts capture. This eliminates: CSP errors from iframe injection, the `uiCommunicator`/`injectCommunicator` message chains, `hideUi`/`showUi`/`toggleClipper` workarounds, timing issues with region capture reactivation, and the entire Mithril dependency for the auth flow. The old sidebar becomes dead code that can be removed incrementally. Also resolves the current CSP-blocked pages issue (#6 in old plan) as a side effect.
